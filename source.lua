if IY_LOADED and not _G.IY_DEBUG == true then
    error("Infinite Yield: Washiez is already running!", 0)
    return
end
...
Title.TextSize = 18
Title.Text = "Infinite Yield: Washiez v" .. currentVersion
...
Credits.Text = "Divided // Sus // MXA // viomod"
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- Infinite Yield: Washiez Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
CMDs[#CMDs + 1] = {NAME = 'audiologger / alogger', DESC = 'Opens a audio logger'}
...
CMDs[#CMDs + 1] = {NAME = 'headless', DESC = 'Removes your head'}
...
CMDs[#CMDs + 1] = {NAME = 'zreach [num]', DESC = 'Increases the Z hitbox of your held tool'}
CMDs[#CMDs + 1] = {NAME = 'xreach [num]', DESC = 'Increases the X hitbox of your held tool'}
...
addcmd('headless',{'nohead'},function(args, speaker)
	for i,v in pairs(speaker.Character:GetChildren()) do
        if v:IsA("BasePart") and v.Name == "Head" then 
            local head = v
            head.Transparency = 1
            for i,x in pairs(head:GetChildren()) do
                if x:IsA("Decal") or x:IsA("Texture") or x:IsA("MeshPart") or x:IsA("SpecialMesh") then 
                    x:Destroy()
                end
            end
        end
...
addcmd('xreach',{},function(args, speaker)
	execCmd('unreach')
	wait()
	for i,v in pairs(speaker.Character:GetDescendants()) do
		if v:IsA("Tool") then
			if args[1] then
				...
				v.Handle.Size = Vector3.new(args[1],0.5,0.5)
				...
				a.Adornee = v.Handle
				v.Handle.Massless = true
				v.Handle.Size = Vector3.new(60,0.5,0.5)
				v.GripPos = Vector3.new(0,0,0)
				speaker.Character:FindFirstChildOfClass('Humanoid'):UnequipTools()
			end
		end
...
addcmd('zreach',{},function(args, speaker)
	execCmd('unreach')
	wait()
	for i,v in pairs(speaker.Character:GetDescendants()) do
		if v:IsA("Tool") then
